...
import type { SearchPaginationItem, SearchViewMode } from '@/types/search';
  ...
  <div className="mx-auto max-w-[1320px] px-4 py-8 sm:px-6">
    <div className="mb-6 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
      <div>
        ...
        <p className="mt-1 text-gray-500">{loading ? 'Завантаження...' : `${filtered.length} пропозицій`}</p>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <button
          type="button"
          className="inline-flex rounded-full bg-gray-900 px-5 py-2.5 text-sm font-semibold text-white"
        >
          <Map size={16} className="mr-2" /> Дивитися на мапі
        </button>
      </div>
    </div>
    ...
    {loading ? (
      ...
      <>
        <div className={viewMode === 'single' ? 'space-y-4' : 'grid grid-cols-1 gap-5 items-stretch xl:grid-cols-2'}>
