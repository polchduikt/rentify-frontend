...
const resolveDisplayName = (profile?: PublicUserProfileDto | null) => {
  const fullName = [profile?.firstName, profile?.lastName].filter(Boolean).join(' ').trim();
  return fullName || FALLBACK_USER_NAME;
};

const resolveInitials = (profile?: PublicUserProfileDto | null) => {
...
const resolvePhone = (profile?: PublicUserProfileDto | null): string => {
